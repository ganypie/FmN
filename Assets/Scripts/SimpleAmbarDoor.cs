...
public class SimpleAmbarDoor : MonoBehaviour, IInteractable
{
    ...
    [Tooltip("Направление открытия: true = положительное (вверх/вправо), false = отрицательное (вниз/влево)")]
    public bool openDirectionPositive = true; // направление открытия
    ...
    private void ToggleDoor(Transform interactor)
    {
        ...
        if (isOpen)
        {
            // Дверь открывается только в одну сторону (настраивается в инспекторе)
            float angle = openDirectionPositive ? openAngle : -openAngle;
            
            // Вращаем по оси X
            targetRot = closedRot * Quaternion.Euler(angle, 0f, 0f);
